document.addEventListener('DOMContentLoaded', () => {
  ...
  let currentSnake = [2,1,0] //3rd div in our grid being 2 (the HEAD), and 0 the end (TAIL,with all 1's being the body from now on)
  let direction = 1
  let score = 0
  let speed = 0.9
  let intervalTime = 0
  ...
  //assign functions to keycodes: moving on divs
  function control(e) {
    squares[currentIndex].classList.remove('snake') //removing the snake's class from ALL the squares.

    if(e.keyCode === 39) {
      direction = 1 //moving right = 1 more div
    } else if (e.keyCode === 38) {
      direction = -width //moving up = 10 minus divs
    } else if (e.keyCode === 37) {
      direction = -1 //moving left = 1 minus div
    } else if (e.keyCode === 40) {
      direction = +width //moving down = 10 more divs
    }
  }
